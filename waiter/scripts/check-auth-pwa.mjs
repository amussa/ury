...
assert.match(app, /maximumQuantity\(item, option\?\.id \?\? null\)/);
assert.match(app, /price_option: line\.price_option/);
assert.match(menu, /const priceOptions = item\.price_options/);
assert.match(menu, /\{priceOptions\.length\} preços/);
assert.doesNotMatch(menu, /price_options\.filter/);
assert.match(menu, /disabled=\{\(stock\.unavailable && !hasMultiplePrices\) \|\| interactionDisabled\}/);
assert.match(menu, /Escolher preço/);
assert.match(editor, /Escolha o preço/);
assert.match(editor, /maximumQuantity\(option\.id\)/);
assert.match(editor, /Stock: \$\{optionMaximum\}/);
assert.match(editor, /disabled=\{unavailable \|\| \(optionSelectionLocked && !selected\)\}/);
assert.match(app, /if \(item\.price_options\.length > 1\) \{\s*setItemEditor\(\{ item, line: null \}\)/);
